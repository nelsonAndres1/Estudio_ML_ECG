# -*- coding: utf-8 -*-
"""
Created on Mon Sep 20 12:08:21 2021
...
def llamado(registr):
    ...
    record=wfdb.rdrecord(f'BD/{registro}',sampto=size)
    annotation = wfdb.rdann(f'BD/{registro}','atr',sampto=size)
...
def detectorQRS(registr):
    regist=100+registr
    registro=str(regist)    
    record = wfdb.rdrecord(f'BD/{registro}', channels=[0], physical=False)
